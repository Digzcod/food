...
const useCityRestaurant = () => {
    ...
    try {
      ...
      const newData =
        data?.data?.cards[1]?.card?.card?.gridElements?.infoWithStyle
          ?.restaurants;
      setHeaderName(data?.data?.cards[11]?.card?.card);
      setCityRestaurants(newData);
      setPLace(data?.data?.cards[1]?.card?.card?.header);
      setLoad(false);
    } catch (error) {
  ...
  return { load, cityRestaurants, place, headerName, scrollRef, handleScroll };
};
